...
function openCreateModal() {
    ...
    document.getElementById('btn-new-folder').addEventListener('click', () => {
        ...
        // Tampilkan Modal "Coming Soon"
        showNoticeModal(
            "Feature Under Development", 
            "We're working hard to bring Folder Organization to VinotesApp. Stay tuned for a more structured workspace!"
        );
    });

    // Fungsi pembantu untuk memunculkan modal pesan profesional
    function showNoticeModal(title, message) {
        const modalHtml = `
            <div id="notice-modal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm">
                <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl max-w-sm w-full mx-4 shadow-2xl transform transition-all">
                    ...
                    <p class="text-slate-400 text-center text-sm leading-relaxed mb-6">${message}</p>
                    <button onclick="document.getElementById('notice-modal').remove()" 
                            class="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-medium transition-colors">
                        Got it!
                    </button>
                </div>
            </div>
